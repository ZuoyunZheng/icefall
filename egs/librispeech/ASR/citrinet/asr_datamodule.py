...
class LibriSpeechAsrDataModule:
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            type=str2bool,
            default=False,
            help="When enabled, select noise from RIRS_NOISES and mix to produce reverb"
            "with training dataset. ",
        )
        group.add_argument(
            "--input-strategy",
    ...
    def train_dataloaders(
        ...
        if self.args.enable_musan:
            ...
            transforms.append(
                CutMix(
                    cuts=cuts_musan, prob=0.75, snr=(5, 10), preserve_id=True
                )
            )
        if self.args.enable_rir:
            logging.info("Enable rir noise")
            logging.info("About to get rir cuts")
            transforms.append(
                ReverbWithImpulseResponse(
                    RecordingSet.from_file(self.args,manifest_dir / "cuts_rir.json.gz"), prob=0.75, snr=(5, 10), preserve_id=True
                )
            )
